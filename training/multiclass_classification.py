...
def train():
    ...
    def create_dynamic_multiclass_dataset(features, labels, class_map, bag_size, batch_size,
        ...
        total_bags = 0
        # Create a separate, dynamically shuffled bag dataset for each class
        for class_id in range(num_classes):
            # 1. Filter instances for the current class
            class_indices = np.where(labels == class_id)[0]
            class_features = features[class_indices]
            num_instances = len(class_features)
            # 2. Create a dataset of INDIVIDUAL INSTANCES for this class
            instance_ds = tf.data.Dataset.from_tensor_slices(class_features)
            # 3. Shuffle instances if this is the training set.
            # `reshuffle_each_iteration=True` ensures new bags every epoch.
            if shuffle_data:
                instance_ds = instance_ds.shuffle(
                    buffer_size=min(num_instances, 20000), 
                    reshuffle_each_iteration=True
                )
            # 4. Group the shuffled instances into bags.
            bag_ds = instance_ds.batch(bag_size, drop_remainder=drop_remainder)
            # 5. Attach the correct one-hot label to each bag
        ...
        # 6. Concatenate the bag datasets from all classes
        full_dataset = all_class_bag_datasets[0]
        for ds in all_class_bag_datasets[1:]:
            full_dataset = full_dataset.concatenate(ds)
        # 7. Shuffle the bags themselves (so batches have a mix of classes)
        if shuffle_data:
            full_dataset = full_dataset.shuffle(
                buffer_size=total_bags, 
                reshuffle_each_iteration=True)
        # 8. Batch the bags for the training steps
    ...
    gc.collect()
    train_dataset, train_steps = create_dynamic_multiclass_dataset(
        X_train, y_train, class_map, 
        cfg.bag_size, cfg.batch_size, cfg.num_classes, 
        shuffle_data=True,  # Enable shuffling for the training set
        drop_remainder=False)
    val_dataset, val_steps = create_dynamic_multiclass_dataset(
        X_val, y_val, class_map, 
        cfg.bag_size, cfg.batch_size, cfg.num_classes, 
        shuffle_data=True, 
        drop_remainder=False)
...
sweep_config = {
    ...
    "parameters": {
        ...
        "bag_size": {"values": [10, 20, 25, 50, 100, 125, 200, 250]},
