...
def calculate_ci_from_log_likelihood(chw_grid_local, sum_log_prob_over_chw_grid_local, fit_window_half_size=4):
    ...
    # Use a fallback to the full range if the window is too small
...
def plot_combined_confidence_intervals(original_cis, corrected_cis, y_labels, title, true_value, bias, x_lim=None):
    ...
    plot_params = {
        ...
        "corrected": {'fmt': 'D', 'label_suffix': 'Bias-Corrected'} # Diamond marker
    }
    for name, data in datasets.items():
        ...
        # Plot points that cover the true value (blue)
        ...
        # Plot points that miss the true value (red)
...
def plot_confidence_intervals_horizontal(ci_data_list, y_axis_labels, title, true_value=0.0, x_lim=None, info_text=None, bag_size=None):
    plt.figure(figsize=(12, max(6, len(ci_data_list) * 0.3))) # Adjust height based on number of CIs
    ...
    plt.tight_layout()
    # plt.savefig(f'0_mult_ci_{bag_size}.png', bbox_inches='tight') 
    plt.show()
    plt.clf()
    plt.close()

# %%
def create_dynamic_multiclass_dataset(features, labels, class_map, bag_size, batch_size,
                                      num_classes, shuffle_data=True, drop_remainder=True):
    all_class_bag_datasets = []
    total_bags = 0
    for class_id in range(num_classes):
        class_indices = np.where(labels == class_id)[0]
        class_features = features[class_indices]
        num_instances = len(class_features)
        instance_ds = tf.data.Dataset.from_tensor_slices(class_features)
        if shuffle_data:
            instance_ds = instance_ds.shuffle(
                buffer_size=min(num_instances, 20000), 
                reshuffle_each_iteration=True
            )
        bag_ds = instance_ds.batch(bag_size, drop_remainder=drop_remainder)
        one_hot_label = tf.one_hot(class_id, num_classes, dtype=tf.float32)
        bag_ds = bag_ds.map(lambda bag: (bag, one_hot_label), 
                                    num_parallel_calls=tf.data.AUTOTUNE)
        #        
        num_class_bags = num_instances // bag_size
        total_bags += num_class_bags
        all_class_bag_datasets.append(bag_ds)
    full_dataset = all_class_bag_datasets[0]
    for ds in all_class_bag_datasets[1:]:
        full_dataset = full_dataset.concatenate(ds)
    if shuffle_data:
        full_dataset = full_dataset.shuffle(
            buffer_size=total_bags, 
            reshuffle_each_iteration=True)
    final_dataset = full_dataset.batch(batch_size, drop_remainder=drop_remainder).prefetch(tf.data.AUTOTUNE)
    steps = total_bags // batch_size if drop_remainder else (total_bags + batch_size - 1) // batch_size
    return final_dataset, steps
...
if cfg.bckgrnd_percnt != 0:
    bg_source = pd.read_parquet(SOME_OTHER_PATH,
                                    engine='pyarrow',
...
all_bag_sizes_all_seed_SMEFT_predictions = {}
...
for bag_size in BAG_SIZE_EXPECTED:
    ...
    del X_train_val, y_train_val, X_train, y_train, X_val, y_val
    gc.collect()
    test_dataset, test_steps = create_dynamic_multiclass_dataset(
        X_test, y_test, class_map, 
        cfg.bag_size, cfg.batch_size, cfg.num_classes, 
        shuffle_data=False, # Disable shuffling for testing
        drop_remainder=False)
    test_dataset = test_dataset.cache()


    y_test_one_hot = np.concatenate([labels.numpy() for _, labels in test_dataset])
    y_test_int = np.argmax(y_test_one_hot,   axis=1)
    class_labels = [str(c) for c in cfg.chw_classes]
    class_id_zero = class_map[0.0]
    mask_zero     = (y_test_int == class_id_zero)
    ...
    for model_idx, model_path in enumerate(model_files):
        ...
        predictions = model.predict(test_dataset,batch_size=5*cfg.batch_size, verbose=1)
        print(f"predictions shape: {predictions.shape}")
        y_pred_int = np.argmax(predictions, axis=1)
        # slice out the rows for true‐0 bags: shape (N0, num_classes)
        pred_zero = predictions[mask_zero]  
        # prepare one list per class j: all P(class=j) for those true‐0 bags
        pred_over_chw_grid = [ pred_zero[:, j] for j in range(pred_zero.shape[1]) ]
        pred_over_chw_grid_np = np.array(pred_over_chw_grid)

        all_models_pred_over_chw_grid.append(pred_over_chw_grid_np)
    ...
    all_bag_sizes_all_seed_predictions[bag_size] = np.copy(all_models_pred_over_chw_grid)
    all_bag_sizes_all_seed_SMEFT_predictions[bag_size] = all_models_predictions
    
    del all_models_predictions, all_models_pred_over_chw_grid, y_test_one_hot, y_test_int, test_dataset
...
# save_path =
# predictions_to_save = {str(key): value for key, value in all_bag_sizes_all_seed_SMEFT_predictions.items()}
# np.savez_compressed(save_path, **predictions_to_save)
# print(f"Successfully saved all prediction data to: {save_path}")
...
for bag_size, all_predictions in all_bag_sizes_all_seed_SMEFT_predictions.items():
    print(f"Bag Size: {bag_size}, Predictions Shape: {all_predictions.shape}")
...
# Iterate over each bag size configuration
for bag_size, all_predictions in all_bag_sizes_all_seed_predictions.items():
    print(f"--- Processing Bag Size: {bag_size} ---")
...
# paper_df.to_csv('mult_detailed_calibration_results.csv', index=False)
...
# def asymptotic_fisher_model(N, I_true, C1, C2):
    # return I_true / (1 + C / N)
    return I_true / (1 + C / np.sqrt(N))
    # return I_true / (1 + C1 / np.sqrt(N) + C2 / (N**2))
...
def plot_fisher_information(ax, data, color, marker, label):
    ...
    try:
        ...
        # initial_guess = [max(mean_info) * 1.1, 1.0, 1.0]
        ...
        # I_true_fit, C1_fit, C2_fit = params
        # I_true_err, C1_err, C2_err = np.sqrt(np.diag(covariance))
        fit_successful = True
    except RuntimeError:
    ...
    if fit_successful:
        x_fit = np.logspace(np.log10(min(bag_sizes)), np.log10(max(bag_sizes) * 1.2), 200)
        y_fit = asymptotic_fisher_model(x_fit, I_true_fit, C_fit)
        # y_fit = asymptotic_fisher_model(x_fit, I_true_fit, C1_fit, C2_fit)
        ax.plot(x_fit, y_fit, linestyle='--', color=color, linewidth=2,
                label=f'{label} Fit ($I_{{True}}={I_true_fit:.1f}$)')


    if label == 'Ensemble Model':
        if fit_successful:
            fit_text = (
                r'$\bf{Fit\ to\ Ensamble\ Model}$' + '\n'
                r'$I_{eff}(N_B) = \frac{I_{true}}{1 + C/\sqrt{N_B}}$' + '\n\n'
                # r'$I_{eff}(N_B) = \frac{I_{true}}{1 + C/N_B}$' + '\n\n'
                f'$I_{{true}} = {I_true_fit:.2f} \pm {I_true_err:.2f}$\n'
                f'$C = {C_fit:.2f} \pm {C_err:.2f}$'
            )
            # fit_text = (
            #     r'$\bf{Fit\ to\ Ensamble\ Model}$' + '\n'
            #     r'$I_{eff}(N_B) = \frac{I_{true}}{1 + C_1/\sqrt{N_B} + C_2/N_B^2}$' + '\n\n'
            #     f'$I_{{true}} = {I_true_fit:.2f} \pm {I_true_err:.2f}$\n'
            #     f'$C_1 = {C1_fit:.2f} \pm {C1_err:.2f}$\n'
            #     f'$C_2 = {C2_fit:.2f} \pm {C2_err:.2f}$'
            # )
            props = dict(boxstyle='round,pad=0.5', facecolor='wheat', alpha=0.7)
            # ax.text(0.05, 0.95, fit_text, transform=ax.transAxes, fontsize=13,
            ax.text(0.05, 0.95, fit_text, transform=ax.transAxes, fontsize=17,
                    verticalalignment='top', bbox=props)
...
ax.set_xscale('log')
ax.set_xlabel('Bag Size ($N_B$)', fontsize=font_size_on_the_sides)
# ax.set_ylabel('Mean Calibrated Fisher Information', fontsize=14)
ax.set_ylabel('Calibrated Fisher Information', fontsize=font_size_on_the_sides)
ax.set_title('Fisher Information vs. Bag Size', fontsize=20, fontweight='bold')
...
# paper_table_values.to_csv('mult_table_values.csv', index=False)
...
SMEFT_ensamble_pred_lits = []
for bag_size, all_predictions in all_bag_sizes_all_seed_SMEFT_predictions.items():
    print(f"--- Processing Bag Size: {bag_size} ---")
    print(f"Bag Size: {bag_size}, Predictions Shape: {all_predictions.shape}")
    ensemble_predictions = np.mean(all_predictions, axis=0)
    SMEFT_ensamble_pred_lits.append(ensemble_predictions)
...
for i, pred_over_chw_grid in enumerate(ensamble_pred_list):
    ...
    plt.tight_layout()
    # plt.savefig(f'0_mult_boxplot_{BAG_SIZE_EXPECTED[i]}.png')
    plt.show()
    plt.close()
...
train_steps = (len(X_train) // cfg.batch_size)
...
# --- Confusion Matrix ---
for i, predictions_prob in enumerate(SMEFT_ensamble_pred_lits):
    print(f'Working on data shape: {predictions_prob.shape} for bag size {BAG_SIZE_EXPECTED[i]}\n\n\n')
    ...
    y_test_one_hot = np.concatenate([labels.numpy() for _, labels in test_dataset])
    y_pred_int = np.argmax(predictions_prob, axis=1)
    y_test_int = np.argmax(y_test_one_hot,   axis=1)
    class_labels = [str(c) for c in cfg.chw_classes]
    # --- Confusion Matrix ---
    all_label_idxs = list(range(len(class_labels)))
    cm = confusion_matrix(y_test_int, y_pred_int, labels=all_label_idxs)
    ...
    plt.tight_layout()
    # plt.savefig(f'0_mult_cm_{BAG_SIZE_EXPECTED[i]}.png', bbox_inches='tight')  # Tight bounding box
    plt.show()
    plt.close()
...
for i, predictions_prob in enumerate(SMEFT_ensamble_pred_lits):
    ...
    print("\n--- Generating Styled ROC Plot ---")
    plt.figure(figsize=(12, 9))
    plt.style.use('seaborn-v0_8-whitegrid')
    # Select which cHW values to plot (focusing on key values)
    chw_to_plot = [chw_grid[len(chw_grid)//2], chw_grid[-len(chw_grid)//4], chw_grid[len(chw_grid)//4], chw_grid[0], chw_grid[-1]]
    plot_classes = []
    for chw_val in chw_to_plot:
        if chw_val in class_map:
            plot_classes.append((chw_val, class_map[chw_val]))
    colors = plt.cm.plasma(np.linspace(0, 1, len(plot_classes)))
    for idx, (chw_val, class_idx) in enumerate(plot_classes):
        fpr, tpr, _ = roc_curve(y_test_one_hot[:, class_idx], predictions_prob[:, class_idx])
        roc_auc = auc(fpr, tpr)
    ...
    plt.title(f'ROC Curves for Selected $c_{{HW}}$ Values (Bag Size: {cfg.bag_size})', fontsize=18)
    plt.legend(loc="lower right", fontsize=16, frameon=True, title="$c_{HW}$ Configurations")
    plt.tick_params(labelsize=12)
    plt.gca().set_aspect('equal', adjustable='box')
    plt.grid(True, which='both', linestyle=':', alpha=0.7)
    plt.tight_layout()
    # plt.savefig(f'0_mult_roc_{BAG_SIZE_EXPECTED[i]}.png', bbox_inches='tight')  # Tight bounding box
    plt.show()
    plt.close()
...
for i, pred_over_chw_grid_np in enumerate(ensamble_pred_list):
    ...
    # Find CI Constant from results_df by finding the value in CI Constant column. We need to select bag size and model type
    ...
    EPSILON = 1e-15
    num_bags_per_chunk = 1000 // cfg.bag_size
    ...
    for chunk_idx, ci_low, ci_high, contains_true in model_chunk_cis[:3]: 
    ...
    if individual_model_cis_all_chunks:
        plot_confidence_intervals_horizontal(individual_model_cis_all_chunks[:50], y_labels_part1[:50], 
                                            #  f'1-Sigma CIs for $c_{{HW}}$ for data chunks (BagSize={cfg.bag_size})',
...
# for j in range(1):
    ...
    for i in range(5):
        ...
        if a_fit < 0:
            ...
            # Confidence intervals using Delta Log-Likelihood
        ...
        plt.tight_layout()
        # plt.savefig(f'0_mult_no_calib_LLR_bag_{cfg.bag_size}_{index_for_next_plot}.png', bbox_inches='tight')  # Tight bounding box
        plt.show()
