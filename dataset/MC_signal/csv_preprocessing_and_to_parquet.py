...
base_dir = "path_to/raw_root_files"

output_file = "integrated_weights.txt"

weights_dict = {}

# Pattern to match cHW value from folder name
chw_pattern = re.compile(r'events_cHW([-+]?\d*\.?\d+)')

# Pattern to match integrated weight line
weight_pattern = re.compile(r'#\s*Integrated weight \(pb\)\s*:\s*([-+]?\d*\.?\d+)')

for folder in sorted(glob.glob(os.path.join(base_dir, "events_cHW*"))):
    chw_match = chw_pattern.search(folder)
    if chw_match:
        chw_value = float(chw_match.group(1))

        banner_file = glob.glob(os.path.join(folder, "*banner.txt"))[0]
        ...
        with open(banner_file, 'r') as f:
            for line in f:
                ...
                if weight_match:
                    ...
                    weights_dict[chw_value] = weight
                    break

with open(output_file, 'w') as f:
    f.write("# cHW_value    Integrated_weight(pb)\n")
    f.write("# --------------------------------\n")
    for chw in sorted(weights_dict.keys()):
        f.write(f"{chw:10.6f}    {weights_dict[chw]:.9f}\n")

print(f"Extracted weights have been saved to {output_file}")

# %%
weights_df = pd.read_csv('integrated_weights.txt', 
                        comment='#', 
                        delimiter=r'\s+', 
                        names=['cHW', 'weight'])

weights_dict = dict(zip(weights_df['cHW'], weights_df['weight']))
weights_dict
...
for file_path in FILE_PATHS:
    ...
    weight_value = weights_dict.get(chw_value)

    if weight_value is None:
        print(f"  Error: No weight found in dictionary for cHW = {chw_value}. Skipping file.")
        continue

    print(f"  Found weight: {weight_value}\n")
...
for file_path in FILE_PATHS:
    ...
    del df, df_encoded, chw_value, weight_value, chw_str
...
output_parquet_file = 'all_data.parquet'
